...
paths.subject = 'EloukMION'; % the subject you want to analyze (eg. if the folder containing the subject data is called 'sub-moumou', omit the prefix 'sub-' and leave only 'moumou')
...
% paths.results_sub_folder = 'Jack20_group2'; % Usefull when computing resulst for a limited number of runs (e.g. jackknife runs). Leave blank '' otherwise.
...
sessions = 2:6; % either a vector or 'all' if you want to analyze all sessions
...
mvt_params.min_dur_steady = 10; % in seconds, minimal duration of a steady period (shorter steady periods will be considered as movement)
...
GLM_params.nPCs.white = 0; % Number of PCs from the PCA on white matter that will be passed as regressors
GLM_params.nPCs.csf = 0; % Number of PCs from the PCA on CSF that will be passed as regressors
GLM_params.GLMdenoise = 1; % use GLM_denoise to derive noise regressors instead of motion parameters
...
paths.L = 1; % Category level at which you want the RDMs to be computed. You can comment or leave '' to compute RDMs at the stimulus level (or indicate the acutal level of the stim level).
...
RDM_params.contrast = 'petkov_a1'; % name of the contrast that will be used to mask the data
RDM_params.peak_threshold = 0.3; % value at which this contrast t-map will thresholded
RDM_params.mask_with_grey = 0; % if set to 1, non-grey matter voxels will be removed from analysis
RDM_params.min_clust_size = 30; % only keep clusters from the contrast t-map that are larger than this value
...
RDM_params.n_peaks = 1; % nb of maxima to keep in each cluster.
