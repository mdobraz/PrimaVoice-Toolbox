clearvars('-except','script_id','recipient','HN','run_index','max_rank','this_params_file'); % clear all but some variables which are used in RunScript or for the jackknife procedure
...
paths.subject = 'John'; % the subject you want to analyze (eg. if the folder containing the subject data is called 'sub-Jim', omit the prefix 'sub-' and leave only 'Jim')
...
paths.results_sub_folder = 'Sensimetrics'; % Usefull when computing resulst for a limited number of runs (e.g. jackknife runs). Leave blank '' otherwise.
...
% Secondary (daily) anatomic file(s). (Anat files acquired at each session used to help func to anat coregistration)
...
%% Functional session and runs to analyze
% There qre different ways to specify sessions & runs:
sessions = [1:2]; % either a vector or 'all' if you want to analyze all sessions
...
% runs = 3; % either a vector (if you want the same run numbers in each session) or 'all' if you want to analyze every run of each session
...
brain_seg.longSyN = 0; % Use long version of antsRegistrationSyN (way way longer, but better)
...
% contrast_names = {'french_vs_all'};
% % contrast weights (the conditions order are displayed for each category level when running this file)
% contrast_weights = [-1 -1 -1  7 -1 -1 -1 -1  0];




% contrast names: better to name them cond1_vs_cond2 (no space allowed)
contrast_names = {'female_vs_male'};
% contrast weights (the conditions order are displayed for each category level when running this file)
contrast_weights = [1  -1  0];
...
RDM_params.peak_threshold = 7; % value at which this contrast t-map will thresholded
...
RDM_params.n_voxels = 57; % size of the max cluster. Examples:
