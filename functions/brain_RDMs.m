%%%%% Compute brain RDMs %%%%%
% Compute RDMs for each local maximum using voxels around the peak value.
%
% Parameters
% ----------
...
for clust = 1:nclust
    ...
    fprintf('\nConstructing RDM...')
    RDM(:,:,clust) = squareform(pdist(thisActivityPattern,pdistance));
    fprintf(' done.\n')
    % %% for each pair
...
%% save RDM file
save(RDM_file,'RDM','cluster','stim_names','subject','-v7');
