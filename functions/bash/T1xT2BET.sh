...
if [[ -n "$CROP_P" ]]; then
  ...
  if test -f "${SCRIPTpath}CropVolume.sh"; then # check if Cropping scripts exists
    bash "${SCRIPTpath}CropVolume.sh" -i $T1 -i $T2 -i $OUT_PREFIX1 -i$OUT_PREFIX2 -b $OUT_PREFIX1 -c $CROP_P -s $CROP_SUFFIX -d $FPOPT
    if [[ $OMASK == "Yes" ]]; then
      bash "${SCRIPTpath}CropVolume.sh" -i "${OUT_PREFIX1}${MASK_SUFFIX}" -b $OUT_PREFIX1 -c $CROP_P -s $CROP_SUFFIX -d $FPOPT
