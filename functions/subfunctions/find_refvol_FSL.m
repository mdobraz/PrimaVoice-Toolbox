function [ref_volume, selected_vols, Q, i1, i2] = find_refvol_FSL(in_file,realign_FSL,mvt_params,paths,mag_file)
...
out_base = fullfile(paths.tmp_nii,['rp_' in_name]);
system(sprintf('%smcflirt -in %s -out %s -refvol 0 -plots -cost %s -dof %i',paths.FSL_prefix,in_file,out_base,realign_FSL.mcflirt_cost,realign_FSL.mcflirt_dof));
delete([out_base ext '.gz'])
out_par = [out_base '.par'];
Q = load(out_par);

%% Convert to SPM-like mvt parameters
qr = rad2deg(Q(:,1:3)); % radians to degrees
Q(:,1:3) = Q(:,4:6); % inverse rotations > translations order
Q(:,4:6) = qr;
...
if any(selected_vols)
    ...
    if exist('mag_file','var') && mvt_params.sel_from_fmap
        ...
        for i = 1:numel(selected_vols)
            if steadiest_period(i)
                ...
                [~,cost] = system(sprintf('%sflirt -in %s -ref %s -schedule ./functions/measurecost1.sch -cost %s | head -1 | cut -f1 -d'' ''',paths.FSL_prefix,roi_out_file,mag_file,realign_FSL.flirt_cost));
    ...
    fprintf('Volume %i was chosen',ref_volume)
else
    ref_volume = nan;
    error('No volume was selected in this run !!!')
end
