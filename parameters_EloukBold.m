...
paths.subject = 'EloukBold'; % the subject you want to analyze (eg. if the folder containing the subject data is called 'sub-moumou', omit the prefix 'sub-' and leave only 'moumou')
paths.task = 'BlocLocalizer'; % the task you want to analyze (eg. if the datafiles include 'task-sparse', omit the prefix 'task-' and leave only 'sparse')
paths.realign_name = 'FSL_mcflirt_dof12_flirt_dof12'; % general name of the analysis (to differentiate analyses that have different parameters)
paths.results_name = '7WM_4CSF_0mvt'; % to distinguish different analyses done of the same realigned data
% paths.results_name = '0WM_0CSF_24mvt'; % to distinguish different analyses done of the same realigned data
% paths.results_name = 'GLM_denoise'; % to distinguish different analyses done of the same realigned data
paths.results_sub_folder = 'MC54'; % Usefull when computing resulst for a limited number of runs (e.g. jackknife runs). Leave blank '' otherwise.

% Main anatomic file(s)
paths.main_anat_suffix = 'T1w0p4mm'; % Suffix of the main structural scan (last part of the anat scan filename [eg. 'T1w'])
paths.main_anat_session = 1; % Session at which the main structural scan was acquired.
paths.T2_anat_suffix = 'T2w0p4mm'; % Suffix of the T2 structural scan (comment line if no T2 available)
...
paths.template_name = 'inia19'; % Template that will be used during the segmentation process. Name should match a 'case' in the switch found in functions/templates.m
...
% sessions = [16 18:26]; % either a vector or 'all' if you want to analyze all sessions
...
%%% if Costmatrix selection has already been launched on all runs, you can use:
% filename = fullfile(fullfile(paths.dataset,['analysis_sub-' paths.subject]),paths.realign_name,'Costmatrix_Run_selection.mat');
% % filename = fullfile(fullfile(paths.dataset,['analysis_sub-' paths.subject]),paths.realign_name,'Costmatrix_Run_selection_2_folds_group2.mat');
% load(filename)
% sessions = selected_sessions;
% runs = selected_runs;

%%% To run the analysis on runs previously selected by R2s or motion methods:
% filename = fullfile(fullfile(paths.dataset,['analysis_sub-' paths.subject]),'R2s_Run_selection_64.mat');
filename = fullfile(fullfile(paths.dataset,['analysis_sub-' paths.subject]),paths.realign_name,'MotionCorr_Run_selection_54.mat');
...
brain_seg.flirt_cost = 'corratio'; % cost function for subject-template registration with flirt (mutualinfo,corratio,normcorr,normmi,leastsq)
...
brain_seg.use_new_segment = 0; % if set to 1 & if subject is human, spm_segment will be used to perform the segmentation (instead of spm_old_segment)
...
realign_FSL.mcflirt_cost = 'corratio'; % cost function for mcflirt (mutualinfo, woods, corratio, normcorr, normmi, leastsquares)
realign_FSL.mcflirt_dof = 12; % degrees of freedom for mcflirt: 6 for rigid body (appropriate in most cases), 7 to compensate for global scale changes, 12 for all degrees of freedom
...
realign_FSL.IR_BETf = 0.3; % -f option of BET. (0->1); default=0.5; smaller values give larger brain outline estimates. 0.5 for human, 0.2 for macaques
realign_FSL.flirt_cost = 'corratio'; % cost function for inter-run registration with flirt (mutualinfo,corratio,normcorr,normmi,leastsq)
realign_FSL.flirt_dof = 12; % degrees of freedom for for inter-run registration with flirt: 6 for rigid body (appropriate in most cases), 7 to compensate for global scale changes, 12 for all degrees of freedom
...
reslice_flags.unwarp = 0; % If set to false, no fieldmap correction will be done
...
mvt_params.max_trans_vox_x = 1; % in voxels, maximum translation allowed between 2 consecutive volumes (in real voxel size)
mvt_params.max_trans_vox_y = 1; % in voxels
mvt_params.max_trans_vox_z = 0.5; % in voxels
mvt_params.trans_fact = 3; % in numbers of max_trans_vox, maximum translation allowed between each volume and the mean of the steadiest period

% Maximum rotations allowed
mvt_params.max_rot_pitch = 0.6; % in degrees (0.3 seems ok), maximum rotation allowed between 2 consecutive volumes
mvt_params.max_rot_roll = 0.3; % in degrees
mvt_params.max_rot_yaw = 0.3; % in degrees
mvt_params.rot_fact = 3; % in numbers of max_rot, maximum rotation allowed between each volume and the mean of the steadiest period

% Steady period definition
mvt_params.min_dur_steady = 5; % in seconds, minimal duration of a steady period (shorter steady periods will be considered as movement)
...
mvt_params.after_mvt_lag = 5; % in seconds, minimal duration to consider as a movement period after a detected movement (can be short for event-related protocol in which the stimulation is movement dependent. Should be around 10 sec for block designs)

% Reference volume selection & Verificaton scan
mvt_params.sel_from_fmap = 0; % Method to select the reference volume of each run:
...
smooth_params.fwhm = [4 4 2]; % in real voxels, Specify the FWHM of the Gaussian smoothing kernel in voxels. Three values should be entered, denoting the FWHM in the x, y and z directions
...
coreg_params.method = 'ants'; % Method used to coregister the average functional volume to the anatomic volume:
...
coreg_params.BETf = 0.3; % -f option of BET for functional average. (0->1); default=0.5; smaller values give larger brain outline estimates. 0.5 for human, 0.3 for macaques
coreg_params.flirt_cost = 'corratio'; % cost function for func-anat registration with flirt (mutualinfo,corratio,normcorr,normmi,leastsq)
coreg_params.use_close2func_anat = 1; % Set to 1 to use an anat scan already close to the func scans to help coregistration
coreg_params.c2f_BETf = 0.3; % -f option of BET for the anat that is close to the funcs. (0->1); default=0.5; smaller values give larger brain outline estimates. 0.5 for human, 0.3 for macaques
...
GLM_params.hrf = [4.5 5.2 10.8 7.35 0.35]; % The parameters of the hrf are specified by a row vector whose elements are:
...
GLM_params.all_trials_are_ok = 0; % set to 1 if each stimulus onset was only recorded when a trial was succesful (trial design).
GLM_params.motion_reg_type = 0; % 0 = no motion regressors (MRs) ; 1 = 6 MRs (trans + rot) ; 2 = 12 MRs (6 + squared) ; 3 = 24 MRs (12 + derviate)
GLM_params.nPCs.white = 7; % Number of PCs from the PCA on white matter that will be passed as regressors
GLM_params.nPCs.csf = 4; % Number of PCs from the PCA on CSF that will be passed as regressors
GLM_params.GLMdenoise = 0; % use GLM_denoise to derive noise regressors instead of motion parameters
GLM_params.GLMdenoise_nPCs.white = 0; % number of PCs from the white matter PCA to use as extraregressors in GLMdenoise
...
paths.CV_results_sub_folder = 'R2s64_group1'; % Results sub-folder where the contrast used for delimiting the clusters (ROIs) is
											  % Use this for cross-validation, comment or leave '' otherwise.
RDM_params.contrast = 'macaque_vs_all'; % name of the contrast that will be used to mask the data
RDM_params.peak_threshold = 3.5; % value at which this contrast t-map will thresholded
RDM_params.mask_with_grey = 0; % if set to 1, non-grey matter voxels will be removed from analysis
RDM_params.min_clust_size = 12; % only keep clusters from the contrast t-map that are larger than this value
RDM_params.n_voxels = 7; % size of the max cluster. Examples:
						  % 1: maximum only, 7: +faces, 19: +edges, 27: +corners
						  % 33: +faces2, 57: +edges2, 81: +corners2, 125: 5-cube
RDM_params.n_peaks = 0.1; % nb of maxima to keep in each cluster.
...
% plot_brain_RDMs(RDM_params.contrast,RDM_params.peak_threshold,RDM_params.mask_with_grey,RDM_params.min_clust_size,RDM_params.n_peaks,RDM_params.n_voxels,paths,ScanLog_ref,coreg_params)
