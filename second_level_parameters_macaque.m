...
% paths.subjects = {'Maga';'Apache';'EloukMION'}; % cell array of the subjects you want to analyze (eg. if the folder containing the subject data is called 'sub-moumou', omit the prefix 'sub-' and leave only 'moumou')
paths.subjects = {'Maga';'Apache'}; % cell array of the subjects you want to analyze (eg. if the folder containing the subject data is called 'sub-moumou', omit the prefix 'sub-' and leave only 'moumou')
...
paths.analysis_name = 'macaque'; % name used to differentiate several second level analyses
% paths.analysis_name = 'macaque_no_MT'; % name used to differentiate several second level analyses
...
paths.CV_results_sub_folder = 'macaques'; % Results sub-folder where the contrast used for delimiting the clusters (ROIs) is. Use this for cross-validation, comment or leave '' otherwise.
...
RDM_params.min_clust_size = 513; % only keep clusters from the contrast t-map that are larger than this value
RDM_params.n_voxels =513; % size of the max cluster. Examples:
