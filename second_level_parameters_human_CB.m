...
% comment to perform the analysis at the stimulus level or to compute brain RDMs
...
RDM_params.peak_threshold = 30; % value at which this contrast t-map will thresholded
RDM_params.mask_with_grey = 0; % if set to 1, non-grey matter voxels will be removed from analysis
RDM_params.min_clust_size = 500; % only keep clusters from the contrast t-map that are larger than this value
RDM_params.n_voxels = 396; % size of the max cluster. Examples:
						  % 1: maximum only, 7: +faces, 19: +edges, 27: +corners
						  % 33: +faces2, 57: +edges2, 81: +corners2, 125: 5-cube
RDM_params.n_peaks = 0.06; % nb of maxima to keep in each cluster.
